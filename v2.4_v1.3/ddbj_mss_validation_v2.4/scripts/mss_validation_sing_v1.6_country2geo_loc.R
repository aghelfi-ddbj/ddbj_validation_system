...
ddbj_mss_validation <- function() {
  line_lines_input_ids <- " wc -l temp_svp/input_ids.txt | cut -f1 -d\" \" "
  ...
  if(lines_input_ids > 0) {
    ...
    input_ids$source <- paste("Rfixed/", input_ids$source,".ann",sep="") # added Rfixed here v1.0
    ...
    for(l in 1:len_input_ids) {
      ...
      if(len_ann_biosample_id > 0) {
        ...
        line_feature_assembly_gap <- "awk -F\"\t\" -v OFS=\"\t\" '{if($1 != \"\") {entry = $1}; if($2 == \"assembly_gap\") {print entry, $2, $3}}' FILE2FIX > temp_autofix/feature_assembly_gap.tsv "
        line_feature_assembly_gap <- sub("FILE2FIX", ann_file2fix, line_feature_assembly_gap)
        line_feature_cds <- "awk -F\"\t\" -v OFS=\"\t\" '{if($1 != \"\") {entry = $1}; if($2 == \"CDS\" || $2 == \"mRNA\") {print entry, $2, $3}}' FILE2FIX > temp_autofix/feature_cds.tsv "
        ...
        line_test_feature_assembly_gap <- " wc -l temp_autofix/feature_assembly_gap.tsv | cut -f1 -d\" \" "
        lines_feature_assembly_gap <- as.numeric(system(line_test_feature_assembly_gap, intern = T))
        line_test_feature_cds <- " wc -l temp_autofix/feature_cds.tsv | cut -f1 -d\" \" "
        lines_feature_cds <- as.numeric(system(line_test_feature_cds, intern = T))
        if(lines_feature_assembly_gap > 0 & lines_feature_cds > 0) {
          feature_assembly_gap <- fread("temp_autofix/feature_assembly_gap.tsv", header = F, sep = "\t")[,c(1, 3)]
          feature_cds <- fread("temp_autofix/feature_cds.tsv", header = F, sep = "\t")[,c(1, 3)]
          ...
          if(len_feature_location > 0) {
            ...
            if(len_n_gap_overlaps > 0) {
              ...
              write.table(agg, "Rfixed/feature_location_aggregated.tsv", row.names=F, col.names=T, quote=F, sep="\t") # not append
              #cat(message_agg)
              all_message_agg <- rbind(all_message_agg, message_agg)
              write.table(all_message_agg, "Rfixed/all_messages_aggregated.tsv", row.names=F, col.names=F, quote=F, sep="\t") # not append
            }
    ...
    write.table(final_log_file, "temp_autofix/out_mss_validation.tsv", row.names = F, col.names = T, quote = F, sep = "\t")
    write.table(final_feature_location, "Rfixed/feature_location.tsv", row.names=F, col.names=T, quote=F, sep="\t")
  }
}

generate_report <- function() {
  line_out_mss_validation <- " wc -l temp_autofix/out_mss_validation.tsv | cut -f1 -d\" \" "
  lines_out_mss_validation <- as.numeric(system(line_out_mss_validation, intern = T))
  if(lines_out_mss_validation > 1) {
    log <- fread("temp_autofix/out_mss_validation.tsv", header = T, sep = "\t")
    ...
    if(len_log > 0){
      ...
      if(len_final_report > 0) {
        ...
        write.table(final_report, "Rfixed/confirmation_report.tsv", row.names = F, col.names = T, quote = F, sep = "\t")
      }
    } else { # len_log
