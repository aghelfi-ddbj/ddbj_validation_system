  ...
  if(lines_input_ids > 0) {
    ...
    for(l in 1:len_input_ids) {
      ...
      if(len_ann_biosample_id > 0) {
        ...
        # awk -F"\t" -v OFS="\t" '{if($1 != "") {entry = $1}; if($2 == "assembly_gap") {print entry, $2, $3}}' /data/Rfixed/SAMD00248959.ann 
        # awk -F"\t" -v OFS="\t" '{if($1 != "") {entry = $1}; if($2 == "CDS" || $2 == "mRNA") {print entry, $2, $3}}' /data/Rfixed/SAMD00248959.ann 
        ...
        if(length_sample_smp_id == 1) {
          ...
          # v1.6 changed feature country to geo_loc_name
          bs_geo_loc_name <- geo_loc_name[geo_loc_name$smp_id == sample_smp_id, "geo_loc_name"]
          len_bs_geo_loc_name <- length(bs_geo_loc_name)
          pos_feature_source <- which(ann$Feature == "source")
          len_pos_feature_source <- length(pos_feature_source)
          first_pos_feature_source <- pos_feature_source[1]
          pos_qualifier_geo_loc_name <- which(ann$Qualifier == "geo_loc_name")
          len_pos_qualifier_geo_loc_name <- length(pos_qualifier_geo_loc_name)

          if(len_pos_qualifier_geo_loc_name > 1) {
            first_pos_qualifier_geo_loc_name <- pos_qualifier_geo_loc_name[2]
            if(pos_qualifier_geo_loc_name[2] > first_pos_feature_source){
              log_file$geo_loc_name_ann <-  ann[first_pos_qualifier_geo_loc_name, ]$Value
            }
          }
          if(len_pos_feature_source >= len_pos_qualifier_geo_loc_name){
...
generate_report <- function() {
  ...
  if(lines_out_mss_validation > 1) {
    ...
    if(len_log > 0){
      final_log_file <- c()
      log$count <- 1
      #len_log <- length(log$source)
      
      # I am here add geo_loc_name, geo_loc_name_ann
      
      
      check <- data.frame(matrix(ncol = 15, nrow = len_log))
      colnames(check) <- c('check_bp', 'check_org', 'check_cnt', 'check_smp', 'check_loc', 'check_coll', 'check_hst', 'check_stra', 'check_isol', 'check_cult', 'check_isosrc', 'check_ovlp', 'check_subsp', 'check_hokasubsp', 'check_geoloc')
      ...
      for(i in 1:len_log) {
        ...
        if(!is.na(log$country[i])) {
          if(!is.na(log$country_source_ann[i])) {
            if(log$country[i] == log$country_source_ann[i]) {
              check$check_cnt[i] <- 0
            } else {
              check$check_cnt[i] <- 1
            }
          } else if(is.na(log$country_source_ann[i])) {
            check$check_cnt[i] <- -1
          }
        } else {
          check$check_cnt[i] <- -2
      ...
      if(len_fix_bp_pos > 0 | len_add_ann_bp_pos > 0 | len_add_meta_bp_pos > 0) {
        ...
        if(len_add_ann_bp_pos > 0) {
          ...
          add_ann_bp_table$type <- "bioproject"
        ...
        if(len_add_meta_bp_pos > 0) {
          ...
          add_meta_bp_table$type <- "bioproject"
      ...
      # country: check_cnt
      fix_cnt_pos <- which(check$check_cnt == 1)
      add_ann_cnt_pos <- which(check$check_cnt == -1)
      add_meta_cnt_pos <- which(check$check_cnt == -2)
      len_fix_cnt_pos <- length(fix_cnt_pos)
      len_add_ann_cnt_pos <- length(add_ann_cnt_pos)
      len_add_meta_cnt_pos <- length(add_meta_cnt_pos)
      fix_cnt_table <- c(); add_ann_cnt_table <- c(); add_meta_cnt_table <- c()
      if(len_fix_cnt_pos > 0 | len_add_ann_cnt_pos > 0 | len_add_meta_cnt_pos > 0) {
        if(len_fix_cnt_pos > 0) {
          fix_cnt_table <- log[fix_cnt_pos, c("source", "biosample_id_ann", "country", "country_source_ann")]
          fix_cnt_table$type <- "country"
          fix_cnt_table$action <- "replace"
        }
        if(len_add_ann_cnt_pos > 0) {
          add_ann_cnt_table <- log[add_ann_cnt_pos, c("source", "biosample_id_ann", "country", "country_source_ann")]
          add_ann_cnt_table$type <- "country"
          add_ann_cnt_table$action <- "add"
        }
        if(len_add_meta_cnt_pos > 0) {
          add_meta_cnt_table <- log[add_meta_cnt_pos, c("source", "biosample_id_ann", "country", "country_source_ann")]
          add_meta_cnt_table$type <- "country"
          add_meta_cnt_table$action <- "missing_biosample"
        }
        report <- rbind(fix_cnt_table, add_ann_cnt_table, add_meta_cnt_table)
        colnames(report)[3:4] <- c("biosample", "ann_file")
        final_report <- rbind(final_report, report)
      }
      ...
      check$qual_subs <- NA # v1.4 bug log$qual_subs
      ...
      test_strain <- check[check$check_stra != -2, ]$qual_subs # v1.4
