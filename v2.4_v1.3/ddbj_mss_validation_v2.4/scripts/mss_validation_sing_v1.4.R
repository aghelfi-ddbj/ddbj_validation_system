  ...
  if(lines_input_ids > 0) {
    ...
    for(l in 1:len_input_ids) {
      ...
      if(len_ann_biosample_id > 0) {
        ...
        # detect overlap (reference: JP0176)
        line_feature_assembly_gap <- "awk -F\"\t\" -v OFS=\"\t\" '{if($2 == \"source\") {entry = $1}; if($2 == \"assembly_gap\") {print entry, $2, $3}}' FILE2FIX > /data/temp_autofix/feature_assembly_gap.tsv "
        line_feature_cds <- "awk -F\"\t\" -v OFS=\"\t\" '{if($2 == \"source\") {entry = $1}; if($2 == \"CDS\") {print entry, $2, $3}}' FILE2FIX > /data/temp_autofix/feature_cds.tsv "
...
generate_report <- function() {
  ...
  if(lines_out_mss_validation > 1) {
    ...
    if(len_log > 0){
      ...
      check <- data.frame(matrix(ncol = 14, nrow = len_log))
      colnames(check) <- c('check_bp', 'check_org', 'check_cnt', 'check_smp', 'check_loc', 'check_coll', 'check_hst', 'check_stra', 'check_isol', 'check_cult', 'check_isosrc', 'check_ovlp', 'check_subsp', 'check_hokasubsp')
