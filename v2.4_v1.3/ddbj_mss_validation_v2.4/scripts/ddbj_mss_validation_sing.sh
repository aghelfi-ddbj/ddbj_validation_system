...
if [[ ${nids} -eq ${nann} && ${nids} -eq ${nfasta} ]]; then
  ...
  sh /mnt/formatlinux_remove-blankspaces_sing_v0.4.sh
  
  ## mss_validation
  echo "Started to check qualifier values from ANN file(s) with DDBJ databases."
  echo "ddbj_mss_validation 2.4" > /data/Rfixed/sing_ddbj_validation.version

  rscript="/mnt/mss_validation_sing_v1.6.R"
  ...
  bash="/mnt/svp_sing_v3.2.sh"
  nohup sh ${bash} > /data/Rfixed/svp.log 2>&1 </dev/null &
  bash_pid=$!
  ...
  # nohup bash -c 'Rscript ${rscript} && sh ${bash}' > /data/Rfixed/mss_validation_svp.log
  
  rm -rf /data/temp_svp/temp_out /data/temp_svp/tmp
  
  # common_check
  echo "Checking COMMON part of ANN file(s)."
  bash2="/mnt/common_check_sing_v0.8.sh" 
  nohup sh $bash2 >/data/Rfixed/common_check.log 2>&1 </dev/null
  bash2_pid=$!
  # sh $bash2 
