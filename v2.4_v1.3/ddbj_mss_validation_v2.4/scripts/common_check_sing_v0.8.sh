    ...
    if test -f /data/temp_svp/common.txt; then
      ...
      if [ ${#biosample_id} -gt 10 ]; then
        ...
        if [ ${len_organism_ann} -gt 0 ] ; then
          # echo "start Rfuzzy"
          Rscript --vanilla /mnt/Rfuzzy_search_taxdump_sing_v0.4.R
          # echo "end Rfuzzy"
        fi
        ...
        if test -f /data/temp_svp/temp_contact_email.txt ; then
          ...
          psubmi=`egrep -n -m1 "SUBMITTER" /data/temp_svp/common.txt | cut -f1 -d":"`
          psubmf=`egrep -n -m1 "street" /data/temp_svp/common.txt | cut -f1 -d":"`
          ...
          if [ ${psubmi} -gt 0 ] && [ ${psubmf} -gt 0 ]; then
            ...
            printf "$ann_filename%s\twarning%s\tCMC0024%s\t$biosample_id%s\n" > /data/temp_svp/common_warning_missing_street.b9
          fi
