  ...
  if test -f "/data/temp_autofix/allfiles2fix.log"; then
      ...
      if [ $test_files2fix -gt 1 ]; then
          while IFS= read -r filename; do
            ...
            #organism=`fgrep -m1 "organism" $filename | cut -f5`
            organism=`awk -F"\t" '$4 == "organism"{print $0;exit}' $filename | cut -f5`
...
#### END of function
# v1.2 fixing bug on autofix_overlap_sing_v0.6.R and /data/Rfixed directory
